...
struct events{
	...
	u8 script_num;
	u8 sign_num;
	struct rom_npc (*npcs)[];//人物
	void* entry_scripts;//出入口
	void* script_scripts;//脚本
	void* sign_scripts;//标志
};

struct mapheader{
    struct footer* map_footer;
    struct events* events;//2026698
    void* scripts;
    void* connections;
    u16 music_id;
    u16 footer_id;
...
struct npc_type{
    ...
    void* formation;
    void* image_anims;
    struct image_resource* gfx_table;
    void* rotscale_anims;
};
