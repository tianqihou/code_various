...
#next feature
BallsExpansion = 		True
FirstNewBallID = 		0x121		#item ID of your first new ball, you need 15 free consecutive item slots
FishingFlag = 	0x270 	#a flag of your choice, works as a way to make game know that you're fighting against a fished pokemon, only for lure ball
#next feature
...
Tutors_Number = 64 						#number of move tutors in your game
TmsHms_Number = 108 					#number of tms + hms in your game
TutorMovesTable = 0x08fd23f4	 	#pointer to move tutor table
TmsMovesTable = 0x08fcd8f4 		#pointer to tms moves table
ExpandedLearnsetTable = True 	#set to true if you're using battle engine's upgrade
LearnsetsTable = 0x08E1B1C4		#pointer to learnsets table in your game
#next feature
...
FlagOptions = 0x0
#next feature
FireRedFishing = True #no biting, you either encounter on the first try or not
ChainFishing = True #XY feature, the more tries you fish on the first try, the more likely you'll encounter a shiny
FishingFlag = 0x270 	#a flag of your choice, works as a way to make game know that you're fighting against a fished pokemon, make sure it's the same as the one you set in the balls expansion(if you expand balls)
#next feature
ShinyCharm = 0x77 #set to item ID if you want to have that item in your hack
#next feature
HallofFameFix = True #set to true if you want to be able to have expanded pokemon properly displayed in the hall of fame
#next feature

insert_offset = 0x1F00600	#Offset as to where insert data FBD500 F70000 0xfc2c7c
...
'''
CFLAGS = ['-mthumb', '-mno-thumb-interwork', '-mcpu=arm7tdmi', '-mtune=arm7tdmi', '-mlong-calls', 
		'-march=armv4t', '-Wall', '-O2']
		
def clear(rom, start, end):
	rom.seek(start)
	rom.write(b'\xFF'*(end - start))
...
def insert_ball_items(rom, table):
	...
	for line in balls:
		...
		#write description
		rom.write((int(descr,16) + 0x08000000).to_bytes(4, 'little'))
...
def insert_script(rom):
	...
	#红线和蛋闪
	hook(rom, table["set_egg_poke"], 0x70260, 1)
	hook(rom, table["egg"], 0x71498, 3)
	...
	if FireRedFishing == True or ChainFishing == True or FishingFlag != 0 or ShinyCharm != 0x0:
		...
		rom.seek(0x0B5734)
		rom.write(bytes.fromhex('30B50006050EFFF723'))
	...
	if NewSpecials == True:
		routine_repoint(rom, table["s25_special"], 0x1DB710)#0x1db67c
